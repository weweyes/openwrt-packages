...
function index()
    ...
    entry({"admin", "services", "filerun"}, cbi("filerun"), _("FileRun"), 1)
    entry({"admin", "services", "filerun", "status"}, call("get_pid")).leaf = true
    entry( {"admin", "services", "filerun", "startstop"}, post("startstop") ).leaf = true
end
